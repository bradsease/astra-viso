...
class test__estimate_instantaneous_angular_rate(starcamtests):
    """
    ...
    def test_central_difference(self):
        ...
        result = self.starcam._estimate_instantaneous_angular_rate(0.5)
    ...
    def test_forward_backward_difference(self):
        ...
        self.starcam.set_pointing_fcn(piecewise_pointing, 'explicit')

        # Forward difference
        result = self.starcam._estimate_instantaneous_angular_rate(0)
        np.testing.assert_almost_equal(result, 0)

        # Backward difference
        result = self.starcam._estimate_instantaneous_angular_rate(1)
        np.testing.assert_almost_equal(result, 0)

class test__estimate_total_angular_displacement(starcamtests):
    """
    ...
    def test_low_fidelity(self):
        ...
        self.starcam.set_pointing_preset("static",
            initial_quaternion=np.array([0, 0, 0, 1]))
        result = self.starcam._estimate_total_angular_displacement(0, 1,
                 precision="low")
        np.testing.assert_almost_equal(result, 0)
